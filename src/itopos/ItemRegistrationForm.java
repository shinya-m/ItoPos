...
public class ItemRegistrationForm extends JDialog{
	...
	public ItemRegistrationForm(final ItemDao dao,Frame owner,final TwitterAccount tw) {
		...
		{
			barcode_label = new JLabel();
			barcode_label.setText("スキャンしてください");
		}
		{
			sold_label = new JLabel();
			sold_label.setText("\u58f2\u5024");
	...
	public void set(Item item){
		barcode_label.setText(item.getBarcode());
		name.setText(item.getName());
		sold_cost.setText(String.valueOf(item.getSold_cost()));
		cost.setText(String.valueOf(item.getCost()));
		num.setText(String.valueOf(item.getPro_num()));
		type_box.setSelectedItem(item.getType());
	}
}
